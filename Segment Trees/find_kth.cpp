...
struct segtree {
	...
	void set(int i, int val, int x, int lx, int rx) {
		...
		if (rx - lx == 1) {
			//If we have reached a leaf, invert it
			sums[x] = val;
			return;
		}
		int mid = (lx + rx) / 2;
	...
	ll findk(ll k, ll x, ll lx, ll rx) {
		// cout << ind << ' ' << x << ' ' << lx << ' ' << rx << endl;
		//If Leaf
		if ( rx - lx == 1) {
			return lx;
		}
		//Else Left or Right Subtree
		ll mid = (lx + rx) / 2;
		ll s_left = sums[2 * x + 1];
		if (k < s_left) {
			return findk(k, 2 * x + 1, lx, mid);
		}
		else {
			return findk(k - s_left, 2 * x + 2, mid, rx);
		}
	}
