...
struct segtree {
	...
	ll first_above(ll val, ll x, ll lx, ll rx) {
		...
		if (rx - lx == 1)
			return lx;
		ll mid = (lx + rx) / 2;
		ll res = first_above(val, 2 * x + 1, lx , mid);
		if (res == -1)
			res = first_above(val, 2 * x + 2, mid, rx);
		return res;
	}

	ll first_above(ll val) {
		return first_above(val, 0, 0 , size);
	}
