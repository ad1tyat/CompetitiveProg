...
struct segtree {
	...
	ll MIN(ll l, ll r, ll x, ll lx, ll rx) {
		if (lx >= r || l >= rx)
		...
		ll mid = (lx + rx) / 2;
		ll s_left = MIN(l, r, 2 * x + 1, lx , mid);
		ll s_right = MIN(l, r, 2 * x + 2, mid , rx);

		return min(s_left, s_right) ; //Partial Overlap
	}
	ll MIN(ll l, ll r) {
