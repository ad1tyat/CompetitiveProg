...
struct segtree {
	...
	item MIN(ll l, ll r, ll x, ll lx, ll rx) {
		if (lx >= r || l >= rx)
			return NEUTRAL_ELEMENT; // Out of Bounds
		if (lx >= l && rx <= r)
			return mins[x];//Completely Inside

		ll mid = (lx + rx) / 2;
		item s_left = MIN(l, r, 2 * x + 1, lx , mid);
		item s_right = MIN(l, r, 2 * x + 2, mid , rx);

		return merge(s_left, s_right) ; //Partial Overlap
	}
	item MIN(ll l, ll r) {
